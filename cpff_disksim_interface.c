#include "cpff_disksim_interface.h"
...
void ssdsim_report_completion(SysTime t, struct disksim_request *r, void *ctx) {
	...
	if(send_request_by_MSQ(KEY_MSQ_DISKSIM_1, re, MSG_TYPE_DISKSIM_1_SERVED) == -1) {
		print_error(KEY_MSQ_DISKSIM_1, "A control request not sent to MSQ in send_request_by_MSQ() return:");
	}
	free(re);
}
...
void hddsim_report_completion(SysTime t, struct disksim_request *r, void *ctx) {
	...
	//ATTENTION!! We ignore the information of each returned request except 'response time'. If you need that for users, you should do something!
	re->responseTime = t-r->start;
	if(send_request_by_MSQ(KEY_MSQ_DISKSIM_2, re, MSG_TYPE_DISKSIM_2_SERVED) == -1) {
		print_error(KEY_MSQ_DISKSIM_2, "A control request not sent to MSQ in send_request_by_MSQ() return:");
	}
	free(re);
}
...
void exec_SSDsim(char *name, const char *parm_file, const char *output_file) {
	...
	//
		...
		if (rp->reqFlag == MSG_REQUEST_CONTROL_FLAG_FINISH) {
			...
			if(send_request_by_MSQ(KEY_MSQ_DISKSIM_1, ctrl, MSG_TYPE_DISKSIM_1_SERVED) == -1) {
				print_error(KEY_MSQ_DISKSIM_1, "A control request not sent to MSQ in send_request_by_MSQ() return:");
			}
			
			disksim_interface_shutdown(disksim, now);
			printf(COLOR_YB"\n[SSDSIM]System Time:%lf, Pending IO Requests:%lu, Served IO Requests:%lu\n"COLOR_RESET, st.stime, st.pendIORequest, st.servedIORequest);
			...
			*/
			...
			if (completed != 0) {
				...
				if (completed != 0) {
					print_error(completed, "[SSDSIM]internal error. Some events not completed:");
					exit(1);
				}
			}
...
void exec_HDDsim(char *name, const char *parm_file, const char *output_file) {
	...
	disksim = disksim_interface_initialize(parm_file, 
																				...
																				0,
																				0);
	print_something(">>>>>[HDDSIM] Launch!");

	REQ *rp = NULL;
	rp = calloc(1, sizeof(REQ));
	for (;;) {
		if(recv_request_by_MSQ(KEY_MSQ_DISKSIM_2, rp, MSG_TYPE_DISKSIM_2) == -1) {
			print_error(-1, "[HDDSIM]A request not received from MSQ in recv_request_by_MSQ():");
		}
		...
		if (rp->reqFlag == MSG_REQUEST_CONTROL_FLAG_FINISH) {
			REQ *ctrl;
				ctrl = calloc(1, sizeof(REQ));
				ctrl->responseTime = st.stime;
				if(send_request_by_MSQ(KEY_MSQ_DISKSIM_2, ctrl, MSG_TYPE_DISKSIM_2_SERVED) == -1) {
					print_error(KEY_MSQ_DISKSIM_2, "A control request not sent to MSQ in send_request_by_MSQ() return:");
				}
				
				disksim_interface_shutdown(disksim, now);
				printf(COLOR_YB"\n[HDDSIM]System Time:%lf, Pending IO Requests:%lu, Served IO Requests:%lu\n"COLOR_RESET, st.stime, st.pendIORequest, st.servedIORequest);
			...
			*/
			...
			if (completed != 0) {
					...
					if (completed != 0) {
							print_error(completed, "[HDDSIM]internal error. Some events not completed:");
							exit(1);
					}
			}
			//Delete pending requests by de-buffering
